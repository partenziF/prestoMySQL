        ...
        public String getResultColumn( String aColumnName , String aColumnAlias ) {
            if ( string.IsNullOrWhiteSpace( TableAlias ) ) {
                if ( !string.IsNullOrWhiteSpace( aColumnAlias ) ) {                    
                    return string.Format( "{0}."+ SQLConstant.COLUMN_NAME_QUALIFIER+"{1}"+ SQLConstant.COLUMN_NAME_QUALIFIER+" AS "+ SQLConstant.COLUMN_NAME_QUALIFIER+"{2}" + SQLConstant.COLUMN_NAME_QUALIFIER , this.TableName , aColumnName , aColumnAlias );
                } else {
                    return string.Format( "{0}."+ SQLConstant.COLUMN_NAME_QUALIFIER+"{1}"+ SQLConstant.COLUMN_NAME_QUALIFIER , this.TableName , aColumnName );
                }
            } else {
                if ( !string.IsNullOrWhiteSpace( aColumnAlias ) ) {
                    return string.Format( "{0}."+ SQLConstant.COLUMN_NAME_QUALIFIER+"{1}"+ SQLConstant.COLUMN_NAME_QUALIFIER+" AS "+SQLConstant.COLUMN_NAME_QUALIFIER+"{2}" + SQLConstant.COLUMN_NAME_QUALIFIER , this.TableAlias , aColumnName , aColumnAlias );
                } else {
                    return string.Format( "{0}."+ SQLConstant.COLUMN_NAME_QUALIFIER+"{1}"+ SQLConstant.COLUMN_NAME_QUALIFIER , this.TableAlias , aColumnName );
                }
            }
        }
